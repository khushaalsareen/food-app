...
const { user } = useUserStore();
  ...
  const toggleStatus = async (rest) => {
    try {
      const emailId = rest.user.email;
      if (rest.user.currentStatus === "active") {
        await axios.post("http://localhost:8000/api/v1/restaurant/block-account", { emailId });
        setRestaurants((prev) =>
          prev.map((r) =>
            r._id === rest._id
              ? { ...r, user: { ...r.user, currentStatus: "blocked" } }
              : r
          )
        );
      } else if (rest.user.currentStatus === "blocked") {
        await axios.post("http://localhost:8000/api/v1/restaurant/unblock-account", { emailId });
        setRestaurants((prev) =>
          prev.map((r) =>
            r._id === rest._id
              ? { ...r, user: { ...r.user, currentStatus: "active" } }
              : r
          )
        );
      }
  ...
  return (
    <div className="max-w-7xl mx-auto p-4">
      ...
      <div className="grid md:grid-cols-3 gap-4">
        {(isSuperadmin ? filteredRestaurants : restaurants).map((rest) => (
          <Card
            key={rest._id}
            className="bg-white dark:bg-gray-800 shadow-xl rounded-xl overflow-hidden hover:shadow-2xl transition-shadow duration-300"
          >
            ...
            {isSuperadmin && (
              <CardFooter className="p-4 border-t dark:border-t-gray-700 border-t-gray-100 flex justify-between items-center">
                ...
                <Button
                  variant={rest.user.currentStatus === "active" ? "destructive" : "default"}
                  onClick={() => toggleStatus(rest)}
                  className="font-semibold py-2 px-4 rounded-full shadow-md transition-colors duration-200"
                >
                  {rest.user.currentStatus === "active" ? "Block" : "Activate"}
                </Button>
              </CardFooter>
            )}
